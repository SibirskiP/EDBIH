...
// ... (ostale metode)
    ...
    public function addUserToRoom($userId)
    {
        ...
        // Autorizacija: Samo admini ili kreator mogu dodavati korisnike
        if (!$this->isCurrentUserAdmin && Auth::id() !== $this->selectedRoom->created_by) {
            session()->flash('error', 'Nemate dozvolu za dodavanje korisnika u ovu grupu.');
            return;
        }
        ...
        if ($user && !$this->selectedRoom->users->contains($user)) {
            $this->selectedRoom->users()->attach($userId, ['is_admin' => false]);
            $this->selectedRoom->load('users'); // Ponovo učitaj relaciju da se osvježi lista
            $this->searchUsers(); // Osvježi listu dostupnih korisnika
            session()->flash('message', $user->username . ' je dodan u grupu.');
            // Opcionalno: pošalji sistemsku poruku u chat
            $this->selectRoom($this->selectedRoom->id); // Refresh chat to reflect changes
        } else {
            session()->flash('error', 'Korisnik je već u grupi ili ne postoji.');
        }
    }
