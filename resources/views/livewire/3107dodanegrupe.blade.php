...
<div x-data="{
            ...
            @elseif($selectedRoom)
                <div class="overflow-y-auto custom-scrollbar flex-grow p-4">
                    ...
                    <div class="flex flex-col items-center mb-6">
                        ...
                        <p class="text-sm text-slate-500 mt-1">Članovi: {{ $selectedRoom->users->count() }}</p>
                    </div>
    ...
    @if($showCreateRoomModal)
        <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" x-data @click.self="$wire.closeCreateRoomModal()">
            <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md" @click.away="$wire.closeCreateRoomModal()">
                <h3 class="text-xl font-bold mb-4">Kreiraj novu sobu</h3>
                <form wire:submit.prevent="createRoom">
                    <div>
                        <label for="newRoomName" class="block text-sm font-medium text-slate-700">Naziv sobe</label>
                        <input type="text" id="newRoomName" wire:model.defer="newRoomName" class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm p-2">
                        @error('newRoomName') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        <button type="button" @click="$wire.closeCreateRoomModal()" class="bg-slate-200 text-slate-700 px-4 py-2 rounded-md hover:bg-slate-300">Odustani</button>
                        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Kreiraj</button>
                    </div>
                </form>
            </div>
        </div>
...
{{-- Potpuno zamijenite stari script blok sa ovim --}}
<script>
    document.addEventListener('livewire:initialized', () => {
        // Sve varijable definišemo na početku da budu dostupne svima unutar ovog bloka
        ...
        // Listener za privatne poruke (ostaje isti)
        if (window.Echo) {
        ...
        // Listener za odabir sobe (sada je unutar 'livewire:initialized' da ima pristup varijablama)
        window.addEventListener('room-selected', event => {
            const roomId = event.detail.roomId;

            // Odjavi se sa starog kanala ako postoji
            if (activeRoomChannel) {
                window.Echo.leave(activeRoomChannel);
            }

            // Ažuriraj naziv aktivnog kanala
            activeRoomChannel = `room.${roomId}`;

            // Pretplati se na novi privatni kanal sobe
            window.Echo.private(activeRoomChannel)
                .listen('RoomMessageSent', (e) => {
                    Livewire.dispatch('incomingRoomMessage', { payload: e });
                })
                .listen('UserTypingInRoom', (e) => {
                    // Ovaj uslov će sada raditi ispravno jer je 'authUserId' vidljiv
