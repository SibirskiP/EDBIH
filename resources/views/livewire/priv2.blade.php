...
<div x-data="{
    ...
    <div class="flex flex-row h-full w-full bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        ...
        <div class="flex flex-col flex-auto h-full p-2 sm:p-4">
            ...
            @if ($selectedUser)
                {{-- ========================================================== --}}
                {{-- OVDJE JE GLAVNI ISPRAVLJENI I OPTIMIZIRANI DIO KODA  --}}
                {{-- ========================================================== --}}
                <div
                    id="chat-container"
                    data-selected-user="{{ $selectedUser->id }}"
                    class="flex flex-col flex-grow overflow-y-auto p-2 sm:p-4 bg-slate-50 rounded-b-xl border border-slate-200 shadow-inner"
                ...
                <form wire:submit="submit" enctype="multipart/form-data" class="p-4 bg-white rounded-b-xl border-t border-slate-200 shadow-md">
                    <div class="flex items-end rounded-xl bg-white w-full px-4 py-2">
                        <div>
                            ...
                            {{-- START MODIFIED PREVIEW SECTION --}}
                            @if($file)
                            ...
                            @endif
                            {{-- END MODIFIED PREVIEW SECTION --}}
                        </div>
                        <div class="flex-grow">
                            <div class="relative w-full">
                                {{-- Optimizacija za "user typing" notifikaciju --}}
                                <textarea
                                    x-ref="messageInput"
            ...
            window.Echo
                ...
                .listen('UserTyping', (event) => {
                    ...
                    if (event.senderId === selectedUserId) {
                        ...
                        if (t) {
                            t.innerText = `${event.senderName ?? 'Korisnik'} tipka...`;
