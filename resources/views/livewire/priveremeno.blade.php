...
<div x-data="{
     ...
     class="flex h-screen antialiased text-slate-800 p-2 sm:p-4 lg:p-8">

    {{-- LIJEVI SIDEBAR (LISTA KORISNIKA) --}}
    <div x-show="sidebarOpen"
         ...
         class="fixed inset-y-0 left-0 z-40 flex flex-col py-6 pl-4 pr-2 w-64 sm:w-72 flex-shrink-0 border-r border-slate-200 bg-slate-50 h-full
                    md:relative md:flex md:translate-x-0 md:opacity-100">
        ...
        <div class="flex items-center justify-between px-4 mb-4">
            ...
            <button @click="activeSidebarView = (activeSidebarView === 'conversations' ? 'rooms' : 'conversations')"
                ...
                <svg x-show="activeSidebarView === 'conversations'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                <svg x-show="activeSidebarView === 'rooms'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            </button>
        </div>
        ...
        <div class="flex-grow overflow-y-auto -mx-2 pr-2 relative">
            ...
            <div x-show="activeSidebarView === 'conversations'"
                 ...
                 x-transition:leave="transition ease-in duration-300 absolute w-full" x-transition:leave-start="opacity-100 transform translate-x-0" x-transition:leave-end="opacity-0 transform -translate-x-4"
                 class="flex flex-col space-y-1">
                @forelse($users as $user)
                    @php
                        $userProfilePic = $user->profilna_slika ? asset('storage/' . $user->profilna_slika) : null;
                        $userInitials = '';
                        if (!$userProfilePic) {
                            $nameParts = explode(' ', $user->username);
                            foreach ($nameParts as $part) { $userInitials .= strtoupper(substr($part, 0, 1)); }
            ...
            <div x-show="activeSidebarView === 'rooms'"
                ...
                <button class="flex flex-row items-center p-3 rounded-xl hover:bg-slate-100 transition-colors duration-200 cursor-not-allowed">
                    <div class="flex items-center justify-center h-10 w-10 rounded-full overflow-hidden flex-shrink-0 bg-gray-300"><svg class="w-6 h-6 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h2a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2h2m3-2v2m0-2a3 3 0 01-3-3V8a3 3 0 013-3h4a3 3 0 013 3v7a3 3 0 01-3 3h-4z" /></svg></div>
    ...
    {{-- GLAVNI PROZOR ZA CHAT --}}
    <div class="flex flex-col flex-auto h-full p-2 sm:p-4">
        {{-- Dugme za otvaranje sidebara na mobilnom --}}
        <button @click="sidebarOpen = true" class="md:hidden p-2 rounded-full hover:bg-slate-100 text-slate-600 mb-4 self-start">
        ...
        <div class="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200 rounded-t-xl shadow-sm mb-4">
            <div class="flex items-center">
                @if ($selectedUser)
                    @php
                        $selectedUserProfilePic = $selectedUser->profilna_slika ? asset('storage/' . $selectedUser->profilna_slika) : null;
                        $selectedUserInitials = '';
                        if (!$selectedUserProfilePic) {
                            $nameParts = explode(' ', $selectedUser->username);
                            foreach ($nameParts as $part) { $selectedUserInitials .= strtoupper(substr($part, 0, 1)); }
                            if (strlen($selectedUserInitials) > 2) { $selectedUserInitials = substr($selectedUserInitials, 0, 2); }
                        }
                    @endphp
        ...
        {{-- ========================================================== --}}
        {{-- OVDJE POČINJE ISPRAVLJENI DIO ZA PRIKAZ PORUKA --}}
        {{-- ========================================================== --}}
        @if ($selectedUser)
            <div
            ...
            >
                ...
                <div
                    ...
                    class="flex flex-col flex-grow overflow-y-auto p-4 bg-slate-50 rounded-b-xl border border-slate-200 shadow-inner space-y-3"
                >
                    @if($hasMore)
                    ...
                    @foreach($messages as $message)
                        ...
                        @if ($message->sender)
                            @php
                                ...
                                $senderProfilePic = $senderUser->profilna_slika ? asset('storage/' . $senderUser->profilna_slika) : null;
                                $senderInitials = '';
                                if (!$senderProfilePic) {
                                    $nameParts = explode(' ', $senderUser->username ?? 'K');
                                    foreach ($nameParts as $part) { $senderInitials .= strtoupper(substr($part, 0, 1)); }
                                    if (strlen($senderInitials) > 2) { $senderInitials = substr($senderInitials, 0, 2); }
                                }
                            @endphp
                                ...
                                <div wire:key="msg-{{ $message->id }}" class="flex justify-end items-end">
                                    <div class="relative max-w-[75%] sm:max-w-xs lg:max-w-md bg-blue-600 text-white py-2 px-4 shadow rounded-xl rounded-br-none">
                                        ...
                                        <div class="flex items-center justify-end text-xs text-blue-200 mt-1">
                                            ...
                                            @if($message->read_at)
                                                {{-- Dvostruka kvačica za pročitano --}}
                                                <svg class="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
                                            @else
                                                {{-- Jedna kvačica za poslano --}}
                                                <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="flex-shrink-0 p-4 bg-white border-t border-slate-200 rounded-b-xl">
                    <form wire:submit.prevent="submit" class="flex items-start space-x-4">
                        <textarea
                            wire:model.defer="newMessage"
                            x-ref="messageInput"
                            rows="1"
                            placeholder="Napiši poruku..."
                            class="flex-1 w-full p-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out resize-none"
                            @input="const el = $el; el.style.height = 'auto'; el.style.height = (el.scrollHeight) + 'px'"
                        ></textarea>

                        <button type="submit" class="p-3 bg-blue-600 text-white rounded-full hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
        ...
        @else
            {{-- Poruka kad nije odabran razgovor --}}
            <div class="flex flex-col flex-auto items-center justify-center h-full text-slate-500">
                <svg class="w-16 h-16 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
                <p class="text-lg font-semibold">Odaberite razgovor da započnete dopisivanje</p>
            </div>
        @endif
