        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    Uredi svoju instrukciju ili kurs
                </h3>
            ...
            <!-- Modal body -->
            <form action="/instrukcije/{{$id}}" method="POST">
                @csrf
                @method('PATCH')
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="vrsta" id="vrsta" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                            <option value="" disabled {{ !$trenutnaVrsta ? 'selected' : '' }}>Izaberi vrstu</option>
                            @foreach($vrste as $vrsta)
                                <option value="{{ $vrsta }}" {{ $vrsta == $trenutnaVrsta ? 'selected' : '' }}>
                                    {{ $vrsta }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" min="5" max="999" name="cijena" id="cijena" value="{{$trenutnaCijena}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="$2999" required="">
                    </div>
                    <div>
                        ...
                        <select name="lokacija" id="lokacija" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                            <option value="" disabled {{ !$trenutnaLokacija ? 'selected' : '' }}>Izaberi lokaciju</option>
                            @foreach($lokacije as $lokacija)
                                <option value="{{ $lokacija }}" {{ $lokacija == $trenutnaLokacija ? 'selected' : '' }}>
                                    {{ $lokacija }}
                                </option>
                            @endforeach
                        </select>
